#!/usr/bin/env python3
"""
Comprehensive test script for VideoReTalking with enhanced pose detection.
...
def run_video_retalking_with_debug(face_video: Path, audio_path: Path, output_path: Path, debug_path: Path):
    """
    Run VideoReTalking inference using Docker with debug output capture.
    ...
    cache_dir = Path.home() / ".cache" / "facexlib" / "weights"
    cache_dir.mkdir(parents=True, exist_ok=True)
    
    # Convert paths to be relative to the workspace for Docker
    workspace_root = Path.cwd()
    face_video_rel = face_video.relative_to(workspace_root)
    audio_path_rel = audio_path.relative_to(workspace_root)
    output_path_rel = output_path.relative_to(workspace_root)
    ...
    cmd = [
        ...
        "cog-video-retalking",
        "python", "inference.py",
        "--face", str(face_video_rel),
        "--audio", str(audio_path_rel),
    ...
    try:
        ...
        debug_content = f"""VideoReTalking Test Debug Output
...
"""
        ...
        logger.info(f"[VIDEO-RETALKING] Debug saved to: {debug_path}")
        
        return True
        
    except subprocess.CalledProcessError as e:
        error_content = f"""VideoReTalking Test Error Output
Generated: {datetime.now().isoformat()}
Face Video: {face_video}
Audio File: {audio_path}
Output Video: {output_path}
...
def main():
    """Test VideoReTalking on all video/audio pairs in test_clips folder."""
    
    test_clips_dir = Path("test_clips")
    results_dir = test_clips_dir / "results"
    ...
    for video_path, audio_path, stem in pairs:
        ...
        try:
            success = run_video_retalking_with_debug(video_path, audio_path, output_video, debug_file)
            if success:
                success_count += 1
